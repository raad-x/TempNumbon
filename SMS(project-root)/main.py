...
# Ring4 Service Constants (HARDCODED as per spec)
RING4_SERVICE_ID = 1574  # Ring4 service ID
RING4_COUNTRY_ID = 1     # United States

# SMSPool API Configuration
SMSPOOL_BASE_URL = "https://api.smspool.net"
SMSPOOL_API_KEY = os.getenv('SMSPOOL_API_KEY') or os.getenv('PROVIDER_API_KEY')
...
if admin_ids_str:
    try:
        ADMIN_IDS = [int(aid.strip())
                     for aid in admin_ids_str.split(',') if aid.strip().isdigit()]
    except (ValueError, AttributeError):
        ADMIN_IDS = []

# Optimized Polling Configuration for Fast OTP Delivery
POLL_INTERVAL = 2        # Base polling interval (adaptive)
POLL_TIMEOUT = 600       # 10 minutes total timeout
ORDER_EXPIRES_IN = 600   # 10 minutes order validity

# Adaptive polling intervals for different phases
POLLING_INTERVALS = {
    'initial': 2,     # First minute: every 2 seconds (fastest)
    'active': 3,      # Next 2 minutes: every 3 seconds
    'standard': 5,    # Next 2 minutes: every 5 seconds
    'extended': 10    # After 5 minutes: every 10 seconds
}

# Performance optimization constants
MAX_CONSECUTIVE_FAILURES = 3  # Before slowing down polling
API_TIMEOUT_SECONDS = 10      # Timeout per API call
MAX_POLLING_INTERVAL = 30     # Maximum interval between polls

# Paths
DATA_DIR = Path("data")
LOGS_DIR = Path("logs")
DB_PATH = DATA_DIR / "ring4_database.json"
    ...
    try:
        ...
        while (datetime.now() - start_time).total_seconds() < POLL_TIMEOUT:
            ...
            try:
                ...
                if result.get('success'):
                    ...
                    if status == 'success' and result.get('sms'):
                    ...
                    # FAILURE STATES: Handle various error conditions
                    elif status in ['cancelled', 'expired', 'timeout', 'failed']:
                        try:
                        ...
                        error_messages = {
                            'cancelled': 'Order was cancelled by the provider',
                            'expired': 'Number validity period expired',
                            'timeout': 'SMS delivery timeout',
                        ...
                        await context.bot.send_message(
                            chat_id=user_id,
                            text=f"❌ <b>{error_msg.title()}</b>\n\n"
                            f"🆔 <b>Order:</b> #{order_id}\n"
                            f"📊 <b>Status:</b> {status}\n"
                            f"⏱️ <b>Duration:</b> {elapsed:.1f}s\n"
                            f"� <b>Polls:</b> {poll_count}\n\n"
                            f"💰 <b>Automatic refund processing...</b>\n"
                            f"Use /refund if needed.",
                            parse_mode='HTML'
                        )

                        user_logger.warning(
                            "❌ Order %s %s after %s polls in %.1fs", order_id, status, poll_count, elapsed)
                        break

                    # PENDING: Continue polling with status update
                    elif status == 'pending':
                        if poll_count % 10 == 0:  # Log every 10th poll to avoid spam
                            api_logger.debug(
                                "🔄 Order %s still pending (poll #%s, %.1fs elapsed)", order_id, poll_count, elapsed)
                    else:
                        api_logger.debug(
                            "🔍 Order %s status: %s (poll #%s)", order_id, status, poll_count)

                else:
                    # API call succeeded but returned error
                    error_msg = result.get('message', 'Unknown API error')
                    api_logger.warning(
                        "⚠️ API error for order %s: %s", order_id, error_msg)
                    consecutive_failures += 1

            except asyncio.TimeoutError:
                consecutive_failures += 1
                api_logger.warning(
                    "⏰ API timeout for order %s (poll #%s)", order_id, poll_count)

                # If too many consecutive failures, slow down polling
                if consecutive_failures >= 3:
                    interval = min(interval * 2, 30)  # Max 30 second interval
                    api_logger.warning(
                        "🐌 Slowing down polling due to failures (new interval: %ss)", interval)
        ...
        try:
            ...
            await context.bot.send_message(
                ...
                f"🔄 <b>Polls Completed:</b> {poll_count}\n\n"
                f"💰 <b>Automatic refund will be processed</b>\n"
